...
>回顾：  
>[webpack初识]('./../1.webpack初识.md)简单介绍了webpack的五大概念，也一起写了两个例子。我们知道，webpack自身只能处理js, json文件，其他类型的文件需要配合loader 或 plugin；我们还知道，loader 和 plugin 需要先用`npm install`下载下来 (当然还有其他的下载方式) 。  
>如果不记得了，可以回去复习一下~
